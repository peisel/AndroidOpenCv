...
public class puzzle15Processor {
     ...
     */
    public synchronized void preparedNewGame()
    {
        do {
            shuffle(mIndexes);
    ...
    /*
    This method is to make the processor know the size of the frame that will be delivered via puzzleFrame
    If the frame will be different size- then the result is unpredictable
     */
    public synchronized void preparedGameSize(int width,int height)
    {
    ...
    /*
    this method to be called from the outside,it processes the frame and shuffles the tiles as specified by mIndexes array
     */
    public synchronized Mat puzzleFrame(Mat inputPicture)
    {
        ...
        for(int i=0;i<GRID_AREA;i++)
        {
            ...
            if (idx==GRID_EMPTY_INDEX)
                ...
                if (mShowTileNumbers){
                    Imgproc.putText(mCells15[i],Integer.toString(i+idx),new Point((cols/GRID_SIZE-mTextWidths[idx])/2,(rows/GRID_SIZE-mTextHeights[idx]+mTextHeights[idx])/2),3,1,new Scalar(255,0,0,255),2);
                }
            }
        }
